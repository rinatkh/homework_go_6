...
- Помни границу buffer: отправка `capacity + 1` ждёт, пока receive не освободит место.
- Не путай закрытый и nil-channel: закрытый пустой канал читается сразу с `ok == false`, операции с nil-channel блокируются.
- Не используй `len(channel)` как механизм синхронизации: это только снимок состояния.
